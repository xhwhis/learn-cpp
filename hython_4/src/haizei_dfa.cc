...
namespace haizei {
    ...
    DFA *DFA::build(ASTree *tree) {
        ...
        switch (tree->type()) {
            ...
            /*case SWITCH: {
                ret->head = new ConditionDFANode(tree->at(0));
                ret->tail = new NopeDFANode();
                ret->head->at(0) = new ConditionDFANode(tree->at(1)->at(0));
                DFA *temp = DFA::build(tree->at(1)->at(1));
                ret->head->at(0)->at(0) = temp->head;
                temp->tail->at(0) = ret->tail;
                ret->head->at(0)->at(1) = new ConditionDFANode(tree->at(2)->at(0));
                for (int i = 2; i < tree->size() - 1; i++) {
                    ret->head->at(0)->at(1) = tem
                    ret->head->at(0) = new ConditionDFANode(tree->at(i)->at(0));
                    DFA *temp = DFA::build(tree(i)->at(i)->at(1));
                    ret->head->at(0) = temp->head;
                    temp->tail->at(0) = ret->tail;
                }
            } break;
            ...
            case BREAK: {
                if (breakPoint.empty()) {
                    throw std::runtime_error("\nbreak statement is wrong! There are no loops.");
                }
                ret->head = ret->tail = new JumpDFANode(breakPoint.top());
            } break;
            case CONTINUE: {
                if (continuePoint.empty()) {
                    throw std::runtime_error("\ncontinue statement is wrong! There are no loops.");
                }
    ...
    DefFunctionDFANode::DefFunctionDFANode(ASTree *tree, DFA *dfa) : DFANode_1(nullptr), func(std::make_shared<FunctionValue>(tree, dfa)) {}
